...
class Trainer(Thread):
    ...
    def run(self):
        ...
        while not self.exit:
            ...
            self.stats_queue.put((SummaryType.SCALAR, "loss/policy", detached_losses["policy"].item()))
    ...
    def _batch_trajectories(cls, trajectories):
        ...
        # (1, batch, hidden)
        lstm_hxs = (
            torch.cat(tensors=[traj.lstm_initial_hidden for traj in trajectories], dim=1),
        ...
        behaviour_actions = torch.stack(tensors=[traj.actions for traj in trajectories], dim=1)
        
        # (seq_len, batch)
        rewards = torch.stack(tensors=[traj.rewards for traj in trajectories], dim=1)
        behaviour_log_probs = torch.stack(tensors=[traj.log_probs for traj in trajectories], dim=1)
